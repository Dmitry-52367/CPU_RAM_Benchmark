...
namespace Load_CPU_RAM
{
    public partial class MainForm : Form
    {
        ...
        Thread[] threads;
        int threadsQuantity = 700000000;
        int priorityOfThreads = 0; // Приоритет процессора для запускаемых тестовых потоков
        ...
        // Метод первичной настройки интерфейса
        ...
        {
            burnLevelOfTest = Convert.ToInt32(nudStressRateValue.Value);
        }
        ...
        // Метод отображения свободной/занятой памяти
        ...
        // Метод отображения загрузки процессора
        ...
        private void btnStart_Click(object sender, EventArgs e)
        {
            ...
            StartCPUStressTest(numberOfStressedCore, threadsQuantity, priorityOfThreads);
        }
        ...
        // Запуск теста CPU
        public void StartCPUStressTest(int numberOfStressedCore, int numberOfThread, int priorityOfThreads)
        {
            ...
            for (int i = 0; i < numberOfStressedCore; ++i)
            {
                ...
                if (priorityOfThreads == 0)
                {
                    threads[i].Priority = ThreadPriority.Lowest; // Ставим минимальный приоритет
                }
                else if (priorityOfThreads == 1)
                {
                    threads[i].Priority = ThreadPriority.BelowNormal; // Ставим приоритет ниже среднего
                }
                else if (priorityOfThreads == 2)
                {
                    threads[i].Priority = ThreadPriority.Normal; // Ставим приоритет обычный
                }
                else if (priorityOfThreads == 3)
                {
                    threads[i].Priority = ThreadPriority.AboveNormal; // Ставим приоритет выше среднего
                }
                else if (priorityOfThreads == 4)
                {
                    threads[i].Priority = ThreadPriority.Highest; // Ставим приоритет высокий
                }
